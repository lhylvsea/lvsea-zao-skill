...
SPEC = importlib.util.spec_from_file_location("lvsea_search_skillsmp", SCRIPT_DIR / "search_skillsmp.py")
...
def run_skills_sh(query: str, timeout: float) -> list[dict[str, Any]]:
    ...
    if completed.returncode != 0:
        detail = strip_ansi(completed.stderr or completed.stdout or "").strip()
        raise RuntimeError(f"skills.sh query failed ({completed.returncode}): {detail[:500]}")
    candidates = parse_skills_sh_output(completed.stdout or "", query)
    if not candidates:
